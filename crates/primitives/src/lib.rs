...
pub use fixed_key_hash::{
    AddressMap, AddressSet, B256Map, FixedKeyBuildHasher, FixedKeyHasher,
};
...
pub unsafe fn copy_address_bytes(dst: *mut u8, src: *const u8) {
    ...
    unsafe {
        if ((dst as usize) | (src as usize)).is_multiple_of(core::mem::align_of::<u64>()) {
            ...
            dst.add(8).cast::<u64>().write(src.add(8).cast::<u64>().read());
            dst.add(16).cast::<u32>().write(src.add(16).cast::<u32>().read());
        } else {
