...
impl Stack {
    ...
    fn top_mut(&mut self) -> *mut U256 {
        ...
        unsafe { self.base_mut().cast::<u8>().add(self.byte_len()).cast::<U256>() }
    }
...
mod tests {
    ...
    fn limits() {
        ...
        assert_eq!(stack.peek(STACK_LIMIT), Err(InstructionResult::StackUnderflow));

        // Overflow: every growing operation refuses, and leaves the stack alone.
        assert!(!stack.push(U256::from(7)));
        assert!(!stack.dup(1));
        assert!(!stack.dup(16));
        assert_eq!(stack.push_slice(&[1]), Err(InstructionResult::StackOverflow));
    ...
    fn stack_clone() {
        ...
        assert_eq!(cloned_empty.is_allocated(), true);
        ...
        assert_eq!(cloned_partial.is_allocated(), true);
        ...
        assert_eq!(cloned_full.is_allocated(), true);
